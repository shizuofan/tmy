...
type Paragraph struct {
	...
	VoiceID    string    `json:"voiceId"`   // 音色ID
	Speed      int       `json:"speed"`     // 语速 [-50, 100], 100=2.0x, -50=0.5x, 0=1.0x
	Volume     int       `json:"volume"`    // 音量 [-50, 100], 100=2.0x, -50=0.5x, 0=1.0x
...
// 支持的语速范围 [-50, 100], 100=2.0x, -50=0.5x, 0=1.0x
const (
	MinSpeed = -50
	MaxSpeed = 100
	DefSpeed = 0
...
// 支持的音量范围 [-50, 100], 100=2.0x, -50=0.5x, 0=1.0x
const (
	MinVolume = -50
	MaxVolume = 100
	DefVolume = 0
...
// ConvertSpeedToRatio 将语速值[-50,100]转换为倍率[0.5,2.0]
func ConvertSpeedToRatio(speed int) float64 {
	if speed <= 0 {
		// -50 -> 0.5, 0 -> 1.0
		return 0.5 + (float64(speed)+50)/100.0*0.5
	}
	// 0 -> 1.0, 100 -> 2.0
	return 1.0 + float64(speed)/100.0*1.0
}

// ConvertVolumeToRatio 将音量值[-50,100]转换为倍率[0.5,2.0]
func ConvertVolumeToRatio(volume int) float64 {
	if volume <= 0 {
		// -50 -> 0.5, 0 -> 1.0
		return 0.5 + (float64(volume)+50)/100.0*0.5
	}
	// 0 -> 1.0, 100 -> 2.0
	return 1.0 + float64(volume)/100.0*1.0
}
